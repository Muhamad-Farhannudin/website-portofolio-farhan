...
export default function index() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("src/data/about.json");
  ...
  return (
    ...
    <div className="w-full md:w-1/2 lg:fixed">
      <div className="inter">
        <h1 className="text-4xl font-bold tracking-tight text-slate-200 ">{aboutData.name}</h1>
        <h2 className="mt-3 md:text-lg font-bold tracking-tight text-slate-200">{aboutData.title}</h2>
        <p className="mt-4 max-w-xs leading-normal text-slate-400">{aboutData.skills}</p>
      </div>
